...
Consequence: `LocalSandboxPaperBrokerAdapter` 默认使用“固定佣金与滑点模型”，买入成交价按参考价上浮 5.00 bps，并收取每笔 1.00 AUD 模拟佣金。`PaperBroker` 把成本计入现金；`/paper/performance/history`、`/paper/broker/status`、dashboard 和 CLI 摘要显示中文成本字段。该模型只用于 paper trading，不改变真实下单禁用边界。
