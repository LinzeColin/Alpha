...
- 影响：控制台、命令行摘要、工单 HTML、中文 CSV 表、FastAPI 元信息、HTTP 错误说明、所有者摘要、审批状态、行情刷新错误、策略候选、模拟经纪商状态/回执和富途牛牛下一步提示均提供中文展示；API 字段名、内部枚举、工单号、路径和股票代码保持机器稳定；`scripts/verify_chinese_display.py` 作为回归门槛。
