...
REASON_TEXT_ZH = {
    ...
    "duplicate idempotency key": "幂等键重复",
    "invalid quantity or price": "数量或价格无效",
    ...
    "external paper broker read only sync unavailable": "外部纸面交易 API 只读同步不可用",
    ...
    "alpaca paper adapter ready": "Alpaca 纸面交易适配器就绪",
...
def zh_strategy_id(value: object) -> str:
    ...
    if raw.startswith("target_rebalance_"):
        return f"目标仓位再平衡 {raw.removeprefix('target_rebalance_')}"
    return raw
